...
joint_mr_p <- plot_grid(
  scatter_p[[1]] + labs(title = "LDL") + theme_bw() + theme(legend.position = 'non', text = element_text(size = 8)),
  scatter_p[[2]] + labs(title = "HDL") + theme_bw() + theme(legend.position = 'non', text = element_text(size = 8)), 
  funnel_p[[1]] + labs(title = "LDL") + theme_bw() + theme(legend.position = 'non', text = element_text(size = 8)), 
  funnel_p[[2]] + labs(title = "HDL") + theme_bw() + theme(legend.position = 'non', text = element_text(size = 8))
  )


legend_scatter <- get_legend(
  # create some space to the left of the legend
  scatter_p[[1]] + theme(legend.box.margin = margin(0, 0, 0, 12))
)

# add the legend to the row we made earlier. Give it one-third of 
# the width of one plot (via rel_widths).
scatter_out.p <- plot_grid(joint_mr_p, legend_scatter, rel_heights = c(3, .4), nrow = 2)
...
MRMV_out <- bind_rows(
    MRMV_ivw_df, MRMV_egger_df, MRMV_wme_df
  ) %>%
