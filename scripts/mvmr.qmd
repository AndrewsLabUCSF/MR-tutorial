...
Due to the high correlation among genetic variants, the GWAS of HDL and LDL may identify loci in the same genomic region. After LD clumping on the smallest p-value from the exposures, we retained `r nrow(comb_exp_independent)` of the initial `r length(mvmr_snps)` SNPs, ensuring that we included only independent variants in the analysis.
